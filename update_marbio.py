...
CATEGORY_RULES = """
STRICT CATEGORY RULES — follow these precisely:
- "african-cdc": ONLY articles about Africa CDC as an institution (strategy, leadership, partnerships, data systems). NOT about specific countries, vaccines, or manufacturing.
- "african-medical-agency": ONLY about AMA, drug regulation, pharmaceutical harmonization in Africa. NOT general Africa CDC news.
- "manufacture-vaccins": ONLY about vaccine/medicine MANUFACTURING and PRODUCTION capacity in Africa (factories, workforce, technology transfer). NOT about vaccine campaigns or distribution.
- "unicef-gavi": ONLY about UNICEF and/or Gavi activities (funding, procurement, immunization campaigns, supply deals). NOT about Africa CDC strategy.
- "sante-maroc": ONLY articles that are specifically and primarily about MOROCCO's health system (hospitals, reforms, medicines, public health in Morocco). An article about Africa CDC that mentions Morocco in passing does NOT belong here.
- "vaccins-monde": ONLY about vaccine development, clinical trials, approvals, and global vaccination campaigns OUTSIDE of Africa CDC institutional news. Focus on WHO, specific diseases, new vaccines, global equity.

CRITICAL RULES:
1. Do NOT take an Africa CDC article and reclassify it into another category by changing the angle.
2. Each article must genuinely belong to its category based on its PRIMARY subject.
3. If a search result doesn't clearly fit a category, DO NOT force it — skip it.
4. If a topic has ALREADY been covered in ANY category (see EXISTING ARTICLES below), do NOT cover it again in a different category. Cross-category duplication is strictly forbidden.
"""
...
def main():
    print("="*50)
    print("Marbio News - Daily Update (v3 — cross-category dedup)")
    print("="*50)
    ...
    print("\nGenerating with Claude (cross-category dedup prompt)...")
    new = call_claude(results, data)
    if not new:
        print("ERROR: No articles generated")
        return

    print("\nMerging with cross-category similarity check...")
    total_added = 0
    total_rejected = 0
